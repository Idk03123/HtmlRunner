...
<body>
    ...
    <header class="bg-[#1a1a1a] p-3 flex flex-wrap gap-3 justify-between items-center border-b border-[#333] z-10">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-md font-semibold text-white">WebForge Cloud</h1>
        </div>
    ...
    <script type="module">
        ...
        const firebaseConfig = {
            apiKey: "AIzaSyCxE1960ZAD1q4H0-QqUdIB_BTE2gcJsv8",
            authDomain: "htmlcoderunner.firebaseapp.com",
            projectId: "htmlcoderunner",
            storageBucket: "htmlcoderunner.firebasestorage.app",
            messagingSenderId: "1047666926082",
            appId: "1:1047666926082:web:525f29e9b1d47bab8bbd1b",
            measurementId: "G-9HSEJZEXVV"
        };

        const app = initializeApp(firebaseConfig);
        const auth = getAuth(app);
        const db = getFirestore(app);
        const appId = 'html-code-runner-github';
        ...
        const initAuth = async () => {
            try {
                // For GitHub pages, we use simple Anonymous sign-in
        ...
        onAuthStateChanged(auth, (user) => {
            currentUser = user;
            if (user) {
                document.getElementById('auth-status').textContent = "Cloud Active (" + user.uid.substring(0,6) + ")";
        ...
        window.saveToCloud = async () => {
            ...
            try {
                // Consistent with Firestore mandatory Rule 1
                ...
                window.closeModals();
                showToast("Saved to your cloud!");
            } catch (e) {
                console.error(e);
                showToast("Error saving (check Firestore rules)", true);
            }
        };
        ...
        if(!editor.value) {
            editor.value = `<h1>WebForge Editor</h1>\n<p>Your cloud storage is now linked!</p>`;
        }
        window.runCode();
    </script>
</body>
